...
func GetItemFromCart() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		pointcursor, err := UserCollection.Aggregate(ctx, mongo.Pipeline{filter_match, unwind, grouping}) // This is the aggregation pipeline to get the total price of the cart
		if err != nil {
			log.Println(err)
		}
		...
		for _, json := range listing {
			c.IndentedJSON(200, json["total"])       // [0]["total] is the total price of the cart"
			c.IndentedJSON(200, filledCart.UserCart) // filledCart.UserCart is the list of items in the cart
		}
		ctx.Done()
	}
...
func (app *Application) BuyFromCart() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.IndentedJSON(200, "Successfully to place order")
	}
